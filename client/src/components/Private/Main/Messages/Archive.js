...
class Archive extends Component {
  ...
  componentDidMount() {
    ...
    this.props.getInitMessages(per, page, "archive");
  }
  ...
  handleScroll = (scrolling, totalPages, page, per) => {
    ...
    if (container && offset && target) {
      ...
      if (result > targetOffset + (container.offsetTop - container.scrollTop)) {
        this.props.getMoreMessages(per, page + 1, "archive");
      }
    }
  };
  ...
  deleteSelected = data => {
    this.setState({ selected: [], checkAll: false });
    this.props.deleteSelected(data, "archive");
  };
  ...
  render() {
    ...
    messages === null || init_loading
      ? (content = <LoaderLarge msg={"Loading archive..."} margin="5vh" />)
      : (content = AllMessages);
    ...
    if (update_loading)
      content = <LoaderSuccess msg={"Moved to inbox."} margin="5vh" />;
...
export default connect(
  mapStateToProps,
  { getInitMessages, deleteSelected, getMoreMessages, moveMessages, loadModal }
)(withRouter(Archive));
